**********************************************************************;
*	411-DL | Unit_01;
*	Last updated: 2015-10-14 by MJG;
**********************************************************************;
...
*	NOTE ON FLAGS: for PROC MI, flag before because PROC MI will REPLACE;
...
data &data_pca.;
	set &data_trans. (drop = &response.: MF_&response.: &key.);
run; quit;
...
*	Three models, two subsets (response and log transform of response): 
	1. Overall Model (Final for Submission)
	2. Training Model
	3. Testing Model;

**********************************************************************;
*	Models: Normal Response;
**********************************************************************;
...
*	Calculate residual: absolute value and square;
data pca_train_response_res;
	set pca_train_response;
...
*	Calculate MAE & MSE;
proc means data = pca_train_response_res mean nway nmiss;
	class train;
	var abs_res square_res;
	output out = pca_train_response_em
	mean(abs_res) = MAE
	mean(square_res) = MSE;
run; quit;

*	View the results;
proc print data = pca_train_response_em;
run; quit;
...
*	Calculate residual: absolute value and square;
data pca_test_response_res;
	set pca_test_response;
...
*	Calculate MAE & MSE;
proc means data = pca_test_response_res mean nway nmiss;
	class test;
	var abs_res square_res;
	output out = pca_test_response_em
	mean(abs_res) = MAE
	mean(square_res) = MSE;
run; quit;

*	View the results;
proc print data = pca_test_response_em;
run; quit;
...
*	Calculate MAE & MSE;
proc means data = pca_train_ln_res mean nway nmiss;
	class train;
	var abs_res square_res;
	output out = pca_train_ln_em
	mean(abs_res) = MAE
	mean(square_res) = MSE;
run; quit;

*	View the results;
proc print data = pca_train_ln_em;
run; quit;
...
*	Calculate MAE & MSE;
proc means data = pca_test_ln_res mean nway nmiss;
	class test;
	var abs_res square_res;
	output out = pca_test_ln_em
	mean(abs_res) = MAE
	mean(square_res) = MSE;
run; quit;

*	View the results;
proc print data = pca_test_ln_em;
run; quit;
...
proc freq data = pca_all_response_OV;
	tables Prediction_Grade;
	title "Operational Validation of TRAIN_&response.";
run; quit;

*	3. Testing;
data pca_test_response_OV;
	set pca_test_response;
...
proc freq data = pca_all_response_OV;
	tables Prediction_Grade;
	title "Operational Validation of TEST_&response.";
run; quit;
...
proc freq data = pca_train_ln_OV;
	tables Prediction_Grade;
	title "Operational Validation of TRAIN_&response._LN";
run; quit;
