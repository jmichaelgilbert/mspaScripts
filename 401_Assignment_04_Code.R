...
# Assignment #4
# Last updated: 2015-05-25 by MJG
...
hospital <- read.csv(file.path("Hospital.csv"), sep=",")
str(hospital)
library(moments)
...
# Look at mean admissions by service type
aggregate(Admissions ~ service, data = hospital, summary)
expType <- aggregate(Tot..Exp. ~ service, data = hospital, summary)

# Initial EDA
summaryAdmissions <- summary(admissions)
summaryBeds <- summary(beds)
summaryExp <- summary(exp)

# Identify the category names available for extraction:
names(summaryAdmissions)
names(summaryBeds)
names(summaryExp)

# Now define IQR for each:
iqrAdmissions <- (summaryAdmissions[["3rd Qu."]][[1]] - summaryAdmissions[["1st Qu."]][[1]])
iqrBeds <- (summaryBeds[["3rd Qu."]][[1]] - summaryBeds[["1st Qu."]][[1]])
iqrExp <- (summaryExp[["3rd Qu."]][[1]] - summaryExp[["1st Qu."]][[1]])

# And create outer fences:
outerfenceAdmissions <- (summaryAdmissions[["3rd Qu."]][[1]] + 3.0 * iqrAdmissions)
outerfenceBeds <- (summaryBeds[["3rd Qu."]][[1]] + 3.0 * iqrBeds)
outerfenceExp <- (summaryExp[["3rd Qu."]][[1]] + 3.0 * iqrExp)

# Print results
print(c(iqrAdmissions, outerfenceAdmissions))
print(c(iqrBeds, outerfenceBeds))
print(c(iqrExp, outerfenceExp))

# Now detect extreme outliers (eo):
eoText <- "Extreme outlier present?"
eoAdmissions <- (summaryAdmissions[["Max."]][[1]] > outerfenceAdmissions)
print(c(eoText, eoAdmissions))
eoBeds <- (summaryBeds[["Max."]][[1]] > outerfenceBeds)
print(c(eoText, eoBeds))
eoExp <- (summaryExp[["Max."]][[1]] > outerfenceExp)
print(c(eoText, eoExp))
...
boxplot(admissions, notch = T, range = 3.0, main = "Admissions", col = "red")
boxplot(beds, notch = T, range = 3.0, main = "Beds", col = "blue")
boxplot(exp, notch = T, range = 3.0, main = "Expenditures", col = "green")
...
par(mfrow = c(2,2))
hist(psy.B, col = "red", xlab = "Beds")
hist(gen.B, col = "green", xlab = "Beds")
boxplot(psy.B, col = "red", ylab = "Beds")
boxplot(gen.B, col = "green", ylab = "Beds")
par(mfrow = c(1,1))
...
# Perform initial EDA
schools <- read.csv(file.path("schools.csv"),sep=",")
...
# Note that the correlation coefficient from cor.test (0.6324) when squared (0.6324^2)
# equals the multiple R-squared value of 0.4 when rounded.  However, this is only a
# preliminary model.  Other factors should be considered.
